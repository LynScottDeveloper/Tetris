...
3. Read the instruction/skill files ONLY if explicitly required for that task.

- CRITICAL: When you encounter a file reference (e.g., @structure or @objective), use your Read tool to load it as .agents/skills/structure/SKILL.md or .agents/skills/objective/SKILL.md. They're relevant to the SPECIFIC task at hand.
- Do NOT preemptively load all references - use lazy loading based on actual need
- When loaded, treat content as mandatory instructions that override defaults
- Follow references recursively when needed

4. Update plan_checklists.txt when a task is completed. Show the checklist to ask for confirmations.
5. Update issues_checklist.txt when:
